...
\end{align}
$$
where $R_{t}$ is the discounted reward starting from timestamp $t$. Finally the loss function is defined as
...
The counter intuitive part here is that we first derive the gradient and then find the surrogate loss function. This is because once we sample $\tau$, we loss the gradient for $\theta$ in $J(\pi_{\theta})$. In order to use backprop, we need to find a surrogate loss function which when taking gradient w.r.t. $\theta$, we get the exact same gradient (another way is to use reparametrization).
...
3. for small $\pi_{\theta}(a_{t}|s_{t})$, taking $\mathbb{log}$ makes the absolute value very big
...
# REINFORCE
Instead of directly computing the expectation, we simply sample trajectories to approximate.

$$
\begin{align}
\mathbb{L}(\theta)
&= -\sum_{t}R_{t}\log \pi_{\theta}(a_{t}|s_{t})
\end{align}
$$
...
REINFORCE is also called score function estimator. Normally, we first define a loss function which is differentiable and rest is handled by deep learning framework for automatically gradient computation.

However, we can still compute the gradient even if we cannot explicitly write the loss function. In other word, as long as we can compute the gradient, we can use gradient descent.

$$
\begin{align}
\frac{\partial}{\partial \theta}\mathbb{E}[f(x)] &= \frac{\partial}{\partial \theta}\int f(x)p_{\theta}(x)dx \\
&= \int f(x)\frac{\partial}{\partial \theta}p_{\theta}(x)dx \\
&= \int p_{\theta}(x) \frac{f(x)}{p_{\theta}(x)}\frac{\partial}{\partial \theta}p_{\theta}(x)dx \\
&= \mathbb{E}_{x\sim p_{\theta}(x)}\left[\frac{f(x)}{p_{\theta}(x)}\frac{\partial}{\partial \theta}p_{\theta}(x)\right] \\
&= \mathbb{E}_{x\sim p_{\theta}(x)}\left[f(x)\frac{\partial}{\partial \theta}\log p_{\theta}(x)\right]
\end{align}
$$

We can directly sample $x$ from $p_{\theta}$ to approximte the gradient. Quite often, when sampling, we might loss the gradient. However, this is not the case here, as we are sampling to approximate the gradient directly. Another benefit is that $f$ can be non-differentiable. In RLVR, $f$ is the reward function which is rarely differentiable.

As mentioned before, the downside of it is that it has high variance. This often leads to training unstablility when using gradient descent.